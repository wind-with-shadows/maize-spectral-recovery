...
parser.add_argument("--data_root", type=str, default='../testdataset/')
...
def main():
    ...
    while iteration<total_iteration:
        ...
        for i, (images, labels) in enumerate(train_loader):
            # print(i)
            # cv2.imshow('1',np.array(images[1,1,:,:]))
            # cv2.waitKey(200)
            # cv2.imshow('1',np.array(labels[1,1,:,:]))
            # cv2.waitKey(200)
            labels = labels.cuda()
            images = images.cuda()
            ...
            optimizer.zero_grad()
            output = model(images)
            # cv2.imshow('1',np.array(output[1,1,:,:].detach().cpu().numpy()))
            # cv2.waitKey(100)
            # cv2.imshow('2',np.array(labels[1,1,:,:].detach().cpu().numpy()))
            # cv2.waitKey(100)
            # loss = criterion_mrae(output, labels)
            lossfuc=torch.nn.MSELoss()
            loss=lossfuc(output,labels)
            loss.backward()
            optimizer.step()
            scheduler.step()
